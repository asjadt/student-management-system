...
trait BusinessUtil
{
    ...
    public function checkManager($id)
    {
        ...
        if (!$user->hasRole(("college_owner" . "#" . auth()->user()->business_id))) {
            return [
                "ok" => false,
                "status" => 403,
    ...
    public function checkEmployees($ids)
    {
        ...
        foreach ($users as $user) {
            ...
            if (!$user->hasRole(("college_owner" . "#" . auth()->user()->business_id)) && !$user->hasRole(("college_owner" . "#" . auth()->user()->business_id)) &&  !$user->hasRole(("business_employee" . "#" . auth()->user()->business_id))) {
                return [
                    "ok" => false,
                    "status" => 403,
    ...
    public function checkStudentStatuses($ids)
    {
        $student_statuses = StudentStatus::whereIn("id", $ids)
            ->get();
        ...
        foreach ($student_statuses as $student_status) {
            if (!$student_status) {
                return [
                    "ok" => false,
                    "status" => 400,
                    "message" => "Student status does not exists."
                ];
            }

            if (auth()->user()->hasRole('superadmin')) {
                if (!(($student_status->business_id == NULL) && ($student_status->is_default == 1) && ($student_status->is_active == 1))) {
                    return [
                        "ok" => false,
                        "status" => 403,
                        "message" => "Student status belongs to another business."
                    ];
                }
            }
            if (!auth()->user()->hasRole('superadmin')) {
                if (!(($student_status->business_id == auth()->user()->business_id) && ($student_status->is_default == 0) && ($student_status->is_active == 1))) {
                    return [
                        "ok" => false,
                        "status" => 403,
                        "message" => "Student status belongs to another business."
    ...
    //           $defaultStudentStatuses = StudentStatus::where([
    //             "business_id" => NULL,
    //             "is_active" => 1,
    //             "is_default" => 1
    //           ])->get();

    //           foreach($defaultStudentStatuses as $defaultStudentStatus) {
    //               $insertableData = [
    //                 'name'  => $defaultStudentStatus->name,
    //                 'color'  => $defaultStudentStatus->color,
    //                 'description'  => $defaultStudentStatus->description,
    //                 "is_active" => 1,
    //                 "is_default" => 1,
    //                 "business_id" => $business_id,
    //               ];
    //            $student_status  = StudentStatus::create($insertableData);
    //            $attached_defaults["student_statuses"][$defaultStudentStatus->id] = $student_status->id;
    //           }
    ...
    //     $default_paid_leave_student_statuses = $defaultSettingLeave->paid_leave_student_statuses>pluck("student_status_id");
    //            $paid_leave_student_statuses_for_business = $default_paid_leave_student_statuses->map(function ($id) use ($attached_defaults) {
    //             return $attached_defaults["student_statuses"][$id];
    // });
    //            $setting_leave->paid_leave_student_statuses()->sync($paid_leave_student_statuses_for_business,[]);



    //            $default_unpaid_leave_student_statuses = $defaultSettingLeave->unpaid_leave_student_statuses()->pluck("student_status_id");
    //            $unpaid_leave_student_statuses_for_business = $default_unpaid_leave_student_statuses->map(function ($id) use ($attached_defaults) {
    //             return $attached_defaults["student_statuses"][$id];
    //  });
    //            $setting_leave->unpaid_leave_student_statuses()->sync($unpaid_leave_student_statuses_for_business,[]);
    ...
    public function loadDefaultSettingLeave($business_id = NULL)
    {
        ...
        foreach ($defaultSettingLeaves as $defaultSettingLeave) {
            ...
            $college_owner_role_id = Role::where([
                "name" => ("college_owner#" . $business_id)
            ])
                ->pluck("id");

            $setting_leave->special_roles()->sync($college_owner_role_id, []);


            $default_paid_leave_student_statuses = $defaultSettingLeave->paid_leave_student_statuses()->pluck("student_status_id");
            $setting_leave->paid_leave_student_statuses()->sync($default_paid_leave_student_statuses, []);

            $default_unpaid_leave_student_statuses = $defaultSettingLeave->unpaid_leave_student_statuses()->pluck("student_status_id");
            $setting_leave->unpaid_leave_student_statuses()->sync($default_unpaid_leave_student_statuses, []);
        }
    ...
    public function loadDefaultAttendanceSetting($business_id = NULL)
    {
        ...
        foreach ($defaultSettingAttendances as $defaultSettingAttendance) {
            ...
            $college_owner_role_id = Role::where([
                ...
                ->pluck("id");
            $setting_attendance->special_roles()->sync($college_owner_role_id, []);
        }
