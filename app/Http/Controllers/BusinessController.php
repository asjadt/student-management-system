...
class BusinessController extends Controller
{
    ...
    public function getAllBusinessesByBusinessOwner(Request $request)
    {
        try {
            ...
            // Check if the user has the 'college_owner' role
            if (!$request->user()->hasRole('college_owner')) {
                // If the user does not have the 'college_owner' role, return a 401 Unauthorized response
                return response()->json([
                    "message" => "You can not perform this action"
                ], 401);
            }
